...
const RangeSlider = ({
  ...
  const handleTrackClick = useCallback((e) => {
    ...
    if (range) {
      ...
      if (minDist <= maxDist) {
        ...
        if (onChange) onChange({ min: newMinValue, max: currentMaxValue });
      } else {
        ...
        if (onChange) onChange({ min: currentMinValue, max: newMaxValue });
      }
    } else {
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (range) {
      if (activeThumb === 'min') {
        ...
        if (onChange) onChange({ min: clampedValue, max: currentMaxValue });
      } else {
        ...
        if (onChange) onChange({ min: currentMinValue, max: clampedValue });
      }
    } else {
      setCurrentValue(newValue);
      if (onValueChange) onValueChange(newValue);
  ...
  const handleKeyDown = useCallback((e, thumbType) => {
    ...
    switch (e.key) {
      ...
      case 'ArrowRight':
        e.preventDefault();
        newValue = range 
          ? (thumbType === 'min' ? currentMinValue : currentMaxValue) + increment
  ...
  const sliderClasses = [
    'ui-range-slider',
    size !== 'medium' && `ui-range-slider--${size}`,
    variant !== 'default' && `ui-range-slider--${variant}`,
  ...
  const generateTicks = () => {
    ...
    return tickValues.map((tickValue, index) => {
      ...
      const isActive = range 
        ? tickValue >= currentMinValue && tickValue <= currentMaxValue
        : Math.abs(tickValue - currentValue) <= step / 2;
      ...
      return (
        ...
        >
          {showTicks && (
            <div className="ui-range-slider-tick-label">
  ...
  return (
    <div className={sliderClasses}>
      {label && (
        <div className="ui-range-slider-label">{label}</div>
      )}
      
      <div className="ui-range-slider-container" ref={containerRef}>
        <div 
          className={`ui-range-slider-track ${disabled ? 'disabled' : ''}`}
          ref={trackRef}
          onClick={handleTrackClick}
        >
          {/* Zone de sélection pour range */}
          {range && (
            <div 
        ...
        {range && (
          ...
          >
            {showTooltipState && activeThumb === 'min' && (
              <div className="ui-range-slider-tooltip visible">
                {formatDisplayValue(currentMinValue)}
              </div>
            )}
          </div>
        )}

        {/* Curseur principal ou max pour range */}
        <div
        ...
        >
          {showTooltipState && (activeThumb === 'max' || !range) && (
            <div className="ui-range-slider-tooltip visible">
              {formatDisplayValue(range ? currentMaxValue : currentValue)}
            </div>
          )}
        </div>
      </div>
      ...
      {showLabels && (
        <div className="ui-range-slider-labels">
          <div className="ui-range-slider-label">
            {minLabel || formatDisplayValue(min)}
          </div>
          
          {showValue && (
            <div className="ui-range-slider-value">
              {range 
                ? `${formatDisplayValue(currentMinValue)} - ${formatDisplayValue(currentMaxValue)}`
          ...
          <div className="ui-range-slider-label">
            {maxLabel || formatDisplayValue(max)}
          </div>
        </div>
      )}
