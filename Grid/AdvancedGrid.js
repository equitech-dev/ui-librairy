...
const AdvancedGridOptimized = ({
  ...
  const gridUtils = useMemo(() => createGridUtils(state.gridMetrics), [state.gridMetrics]);

  // Détecteur de collision mémorisé
  const collisionDetector = useMemo(() => createCollisionDetector(gridRef, gridUtils), [gridRef, gridUtils]);
  ...
  const handleMouseMove = useCallback(event => {
    ...
    if (state.isResizing) {
      dispatch({
        type: GRID_ACTIONS.UPDATE_RESIZE,
        payload: {
          mouse
        }
      });
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (state.isDragging) {
      dispatch({
        type: GRID_ACTIONS.END_DRAG
      });
      handleDrop();
    }
    if (state.isResizing) {
      dispatch({
        type: GRID_ACTIONS.END_RESIZE
      });
      handleResizeEnd();
    }
  ...
  const handleDrag = useCallback(event => {
    ...
    const {
      col,
      row
    } = gridUtils.mouseToCell(event.clientX, event.clientY);
    const newState = {
  ...
  const handleResizeEnd = useCallback(() => {
    ...
    const newState = gridUtils.getElementState(state.active);
    onItemResize(state.active.dataset.id, {
      w: newState.w,
      h: newState.h
    });
  ...
  useEffect(() => {
    const updateMetrics = () => {
      ...
      if (metrics) {
        dispatch({
          type: GRID_ACTIONS.SET_GRID_METRICS,
          payload: metrics
        });
      }
    };
